...
void runSweep() {
  ...
  while ((readData(STATUS_REG) & 0x07) < 4 ) { // Check that status reg != 4, sweep not complete
    ...
    if (flag == 2) {
      ...
      re = (R1 << 8) | R2; // reading the real component

      R1  = readData(IMG_DATA_R1);
      R2  = readData(IMG_DATA_R2);
      img = (R1 << 8) | R2; // reading the img component

      freq = start_freq + i * incre_freq; // calculating the current frequency, which is the start_frequency + current_iteration * frequency_increment
      mag = sqrt(pow(double(re), 2) + pow(double(img), 2)); // calculating the magnitude
      tot = tot + mag; // accumulating magnitude for averaging
      magcount = magcount + 1; // same purpose as above
      GF = (1 / .22) / 19.06; // Gain factor calculation, refer to page 17 of datasheet
      Impedance = 1 / (GF * mag); // impedence calculation, refer to page 17
      if (Impedance < 100) { // TODO: figure out the output range of impedance
        impcount = impcount + 1; // accumulating impedance for averaging
